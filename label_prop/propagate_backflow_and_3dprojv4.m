function propagate_backflow_and_3dprojv4(video_name)
...
%label_dir       = [src_dir video_name '/label/'];
label_dir       = [src_dir video_name '/label10/'];
...
% loads all the keyframes
%load([src_dir video_name '/allKeyframes.mat'], 'keyframes');

load([src_dir video_name '/train+valKeyframes.mat'], 'trainKeyframes');
...
% border case for (hotel_umd)
% start = 1866;
% kfIndex = 82;

% border case for (studyroom)
% start = 3321;
% kfIndex = 49;
...
for nn = start:length(allFileNumbers)-1
...
%         load('tmp.mat', 'labelP');
        elseif (nLabel == 10)            
            labelP = load(labelPpath, 'mapLabel10');
            labelP = labelP.mapLabel10;
        elseif (nLabel == 13)            
...
% % %         end
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%        
        if ( j > keyframes(kfIndex)  ) % push the kfIndex to point to the furthest end of the two keyframes            
            kfIndex = kfIndex + 1;                                       
        end
        
        kfList = keyframes(kfIndex-1); 
        if (kfIndex <= length(keyframes))
            kfList = cat(2, kfList, keyframes(kfIndex));            
        else
            kfIndex = kfIndex - 1;     % adjust the incorrect increment made in hte previous block
            % only if there is one more keyframe available (other wise use only the nearest end frame only)
        end
        
        windowLen = length(kfList); % if there is only one hand annotated-frame left (usually at the end of the video) then window will be 1 otherwise 2.
        ...
        for ikfList=1:length(kfList)
            ...
            if (nLabel == 4)
                tmpLabelP = load(labelPpath, 'superLabel');
            elseif (nLabel == 10)            
                tmpLabelP = load(labelPpath, 'mapLabel10');
                tmpLabelP.superLabel = tmpLabelP.mapLabel10;  
                tmpLabelP.mapLabel10 = [];
            elseif(nLabel == 13)
...
% % % %     
    ...
    system(['chmod 777 ' label_dir_pred strName2 '.mat']);
